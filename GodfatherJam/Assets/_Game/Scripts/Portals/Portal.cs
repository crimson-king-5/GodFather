...
public class Portal : MonoBehaviour {
    ...
    public float portalEjectionForce;
    public float portalEjectionDuration;
    ...
    void HandleTravellers () {
        //if(trackedTravellers.Count == 1)
        //{
        //    PortalTraveller travellerA = trackedTravellers[0];
        //    Debug.Log("traveller A : " + trackedTravellers[0].name + " pos : " + trackedTravellers[0].transform.position);
        //}
        ...
        for (int i = 0; i < trackedTravellers.Count; i++) {
            ...
            Vector3 offsetFromPortal = travellerT.position - screen.gameObject.transform.position;

            //Debug.Log("offset from portal : " + offsetFromPortal);

            int portalSide = System.Math.Sign (Vector3.Dot (offsetFromPortal, screen.gameObject.transform.forward));

            //Debug.Log("portal side : " + portalSide);

            int portalSideOld = System.Math.Sign (Vector3.Dot (traveller.previousOffsetFromPortal, screen.gameObject.transform.forward));

            //Debug.Log("portal side old : " + portalSideOld);

            // Teleport the traveller if it has crossed from one side of the portal to the other
            if (portalSide != portalSideOld) {
                var positionOld = travellerT.position;
                var rotOld = travellerT.rotation;
                traveller.Teleport (screen.gameObject.transform, linkedPortal.transform, m.GetColumn (3), m.rotation);
    ...
    public enum TicknessAxis { X, Y, Z};
    public TicknessAxis ticknessAxis;
